function extract_CONN_atlas_FC(sublist,CONN_x,atlas,dataset,run_tag)
...
% run_tag (optional)    : string specifying which run to use (when there are multiple runs in a session)
% OUTPUTS:
...
for i=1:length(sublist)
    ...
    error(['ERROR: Multiple runs in session: re-rerun with run_tag specified']);
    end
    fd=fd.R;
...
save([datapath filesep dataset filesep 'Group' filesep 'meanFD_runs' run_tag],'mean_FD');
mean_FD=mean(mean_FD,2);
save([datapath filesep dataset filesep 'Group' filesep 'meanFD' run_tag],'mean_FD');
