function CPM_view_networks(pos_mask,neg_mask,atlas,norm)

% Extract top intra- and inter-network contributors to MW CPM
% Inputs:
% pos_mask  : binary positive mask (matrix)
% neg_mask  : binary negative mask (matrix)
% atlas     : 1=Shen268, 2=Schaefer300 (7Networks), 3=Schaefer300 (17Networks)
% norm      : 0 = no normalization; 1 = normalize # of features per network pair by total # of network pairs
...
elseif atlas==3
fig=figure('Position',[0 0 1650 800]);
end
ax(1)=subplot(1,2,1);
...
%title({[' '];[' '];['CPM Positive Features'];[' ']},'Fontweight','normal');
hold on;
cut=1; cut2=-1;
for i=1:nx+1
...
c.FontSize=14;
c.Label.String='Number of Edges';
%title({[' '];[' '];['CPM Negative Features'];[' ']},'Fontweight','normal');
hold on;
cut=1; cut2=-1;
for i=1:nx+1
    plot([0.5,cut+0.5],[i-.5,i-.5],'k-');
